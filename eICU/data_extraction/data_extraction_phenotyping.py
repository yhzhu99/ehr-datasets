...
def data_extraction_phenotyping(args):
    ...
    all_df = utils.embedding(args.root_path)  # Done
...
def main():
    ...
    pheno, pheno_label = data_extraction_phenotyping(args)
    pheno.to_csv(os.path.join(args.root_path, 'phenotyping.csv'), index=False)
    pheno_label.to_csv(os.path.join(args.root_path, 'phenotyping_label.csv'), index=False)
